...
struct thread *thread_create(void (*f)(void *), void *arg, int execution_time_slot){
    ...
    if( is_thread_start == 0 ){
        ...
        //新加的
        t->last_remain_execution_time = t->remain_execution_time;
    }
...
void schedule(void){
    ...
    /*    int current_id = current_thread->ID;
        current_thread->last_remain_execution_time = current_thread->remain_execution_time;
    ...
    {   
        int executed_time = current_thread->last_remain_execution_time - current_thread->remain_execution_time;
        if(executed_time / __time_slot_size == 3){
            current_thread->last_remain_execution_time = current_thread->remain_execution_time;
            current_thread = current_thread->next;
        //    current_thread->last_remain_execution_time = current_thread->remain_execution_time;
        }
...
void thread_start_threading(int time_slot_size){
    ...
    while (tmp_thread != NULL)
    {
        ...
        //新加的
        tmp_thread->last_remain_execution_time = tmp_thread->remain_execution_time;
